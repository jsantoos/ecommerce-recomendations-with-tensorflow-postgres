...
async function trainModel({ users }) {
    ...
    context.productVectors = products.map(product => {
        return {
            ...
            meta: { ...product },
            vector: Array.from(
                encodeProduct(product, context).dataSync()
            )}
    })
    ...
    await fetch("http://localhost:3001/vectors/index", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
        body: JSON.stringify({
            products: context.productVectors
        })
    })
    console.log("Vector length:", context.productVectors[0].vector.length)
...
async function recommend({ user }) {
    ...
    console.log("User vector size:", userVector.length);

    const response = await fetch("http://localhost:3001/vectors/search", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
    ...
    console.log("Nearest products:", nearest);

    const candidateProducts = context.productVectors
        .filter(p =>
            nearest.some(n => n.product_id === p.name)
        );

    console.log("Candidates:", candidateProducts.length);

    // 🚨 Se não houver candidatos
    if (candidateProducts.length === 0) {
        console.warn("No candidates from vector DB.");
        postMessage({
